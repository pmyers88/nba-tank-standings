...
const messages = {
  ...
  getTankRankingsMessage: (teams) => {
    return `The top${teams.length > 1 ? ' ' + teams.length : ''} ${pluralize('team', teams.length)} in the tank rankings ` +
        `${pluralize('is', teams.length)} ${compoundSubject(teams).delimitAll().make()}.`;
  }
};
